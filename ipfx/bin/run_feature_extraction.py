...
import ipfx.nwb_writer as writer

import ipfx.plot_qc_figures as plotqc
import ipfx.logging_utils as lu


def embed_spike_times_old(input_nwb_file, output_nwb_file, sweep_features):
...
def embed_spike_times(input_nwb_file, output_nwb_file, sweep_spikes):
    ...
    nwb_data = writer.create_nwb_writer(tmp_nwb_file)
    for sweep_num,spike_times in sweep_spikes.items():
        nwb_data.add_spike_times(sweep_num, spike_times)
